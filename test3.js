...
let symbolSubscribers = {}; // Map symbol -> Set of user clients
let indicesSubscription = new Set(); // Track indices separately
let lastKnownData = {}; // Store last known data for all users

function updateSubscription(symbols, res) {
    symbols.forEach(symbol => {
        if (!symbolSubscribers[symbol]) {
            symbolSubscribers[symbol] = new Set();
        }
        symbolSubscribers[symbol].add(res);
...
function logSubscriptions() {
    ...
    Object.entries(symbolSubscribers).forEach(([symbol, users]) => {
        console.log(`📊 Symbol: ${symbol}, Connected Users: ${users.size}`);
    });
}
...
fyersdata.on("connect", () => {
    ...
    if (indicesSubscription.size > 0) {
        ...
        console.log(`📈 Indices Data Subscribed: ${Array.from(indicesSubscription).join(", ")}`);
    }
});
...
fyersdata.on("message", (message) => {
    ...
    console.log(`📊 Received Data: ${message.symbol} -> LTP: ${message.ltp}`);

    // Send data to all clients subscribed to this symbol
    if (symbolSubscribers[message.symbol]) {
        symbolSubscribers[message.symbol].forEach(client => {
            client.write(`data: ${JSON.stringify(filteredData)}\n\n`);
        });
    }
});

// API to Subscribe to Symbols
app.post("/data/:category", (req, res) => {
    ...
    if (category === "indices") {
        ...
        fyersdata.subscribe(Array.from(indicesSubscription));
        console.log(`📈 Indices Subscribed: ${symbols.join(", ")}`);
    } else {
        updateSubscription(symbols, res);
    }

    res.json({ message: `${category} data updated successfully` });
});

// SSE Endpoint to Stream Data
app.get("/subscribe", (req, res) => {
    ...
    const userSymbols = symbols.split(",");
    updateSubscription(userSymbols, res);

    // Send last known data immediately
    userSymbols.forEach(symbol => {
        if (lastKnownData[symbol]) {
            res.write(`data: ${JSON.stringify(lastKnownData[symbol])}\n\n`);
        }
    });
    ...
    req.on("close", () => {
        userSymbols.forEach(symbol => {
            if (symbolSubscribers[symbol]) {
                symbolSubscribers[symbol].delete(res);
        ...
        console.log(`❌ User disconnected from: ${userSymbols.join(", ")}`);
    });

    console.log(`✅ User subscribed for: ${userSymbols.join(", ")}`);
});
...
app.post("/unsubscribe-category", (req, res) => {
    ...
    symbols.forEach(symbol => {
        if (symbolSubscribers[symbol]) {
            symbolSubscribers[symbol].clear();
            delete symbolSubscribers[symbol];
        }
    });

    updateUnsubscription();
    res.json({ message: "Unsubscribed from symbols successfully" });
});
