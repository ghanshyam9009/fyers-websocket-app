...
function updateSubscription(symbols, userId) {
    ...
    if (newSymbols.length > 0) {
        ...
        newSymbols.forEach(symbol => {
            subscribedSymbols.add(symbol);
            symbolSubscribers[symbol] = symbolSubscribers[symbol] || new Set();
        });
        console.log(`✅ Subscribed to new symbols: ${newSymbols}`);
    }
    symbols.forEach(symbol => {
...
function updateUnsubscription() {
    for (const symbol of [...subscribedSymbols]) {
        const stillNeeded = Object.values(symbolSubscribers[symbol] || {}).size > 0;
        if (!stillNeeded) {
            fyersdata.unsubscribe([symbol]);
            subscribedSymbols.delete(symbol);
            delete symbolSubscribers[symbol];
...
fyersdata.on("message", (message) => {
    ...
    (symbolSubscribers[message.symbol] || []).forEach(userId => {
        userSessions[userId]?.clients.forEach(client => {
            client.res.write(`data: ${JSON.stringify(filteredData)}\n\n`);
        });
        console.log(`📤 Sent ${message.symbol} data to User ${userId}`);
    });
});
